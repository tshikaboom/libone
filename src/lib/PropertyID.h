...
#ifndef INCLUDED_PROPERTYID_H
#define INCLUDED_PROPERTYID_H
...
class PropertyID {
  public:
    PropertyID(uint32_t new_value);
    uint32_t get_value();
    uint32_t get_id();
    uint32_t get_type();
    uint16_t get_bool_value();
...
WebPictureContainer2398 = 0x200034C8,
      value_NIL = 0
    } property_value = value_NIL;
